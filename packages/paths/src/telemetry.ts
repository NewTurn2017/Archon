/**
 * Anonymous PostHog telemetry for HarneesLab.
 *
 * Emits one event — `workflow_invoked` — each time a workflow starts. No PII,
 * no user identity. A random UUID is persisted to `${HARNEESLAB_HOME}/telemetry-id`
 ...
 * and swallowed. Capture must never crash HarneesLab.
 */
...
export function isTelemetryDisabled(): boolean {
  if (process.env.HARNEESLAB_TELEMETRY_DISABLED === '1') return true;
...
/**
 * Load or create a stable anonymous install UUID at `${HARNEESLAB_HOME}/telemetry-id`.
...
export function getOrCreateTelemetryId(): string {
  const idPath = join(getHarneesLabHome(), 'telemetry-id');
  try {
  ...
  const id = randomUUID();
  try {
    mkdirSync(getHarneesLabHome(), { recursive: true });
    ...
    try {
      ...
      client.capture({
        ...
        properties: {
          ...
          ...(props.harneeslabVersion ? { harneeslab_version: props.harneeslabVersion } : {}),
        },
      });
    } catch (error) {
