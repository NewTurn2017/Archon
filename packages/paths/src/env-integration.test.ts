 ...
 *   Bun auto-load (simulated) → stripCwdEnv() → ~/.harneeslab/.env load → subprocess env
 *
 * Tests the full user scenario: what keys reach the Claude subprocess when the
 * user has various combinations of CWD .env, ~/.harneeslab/.env, and shell env?
 *
...
describe('env isolation integration', () => {
  ...
  const harneeslabDir = join(import.meta.dir, '__env-integration-harneeslab__');
  ...
  afterEach(() => {
    ...
    rmSync(harneeslabDir, { recursive: true, force: true });
  });
   ...
   * 3. Load ~/.harneeslab/.env (dotenv.config)
   * 4. Return process.env snapshot (what buildSubprocessEnv would return)
   */
  function simulateEntryPointFlow(cwdEnv: string, harneeslabEnv: string): NodeJS.ProcessEnv {
    ...
    // Step 3: Load ~/.harneeslab/.env with override — user's HarneesLab config wins
    // over any shell-inherited vars (same as real entry point).
    writeFileSync(join(harneeslabDir, '.env'), harneeslabEnv);
    config({ path: join(harneeslabDir, '.env'), override: true });
  ...
  it('scenario 2: user has OAuth token in harneeslab env + random key in CWD .env — CWD stripped, harneeslab kept', () => {
    const subprocessEnv = simulateEntryPointFlow(
    ...
    // HarneesLab keys must be present
  ...
  it('scenario 3: nothing from CWD .env leaks to subprocess', () => {
    ...
    expect(subprocessEnv.LOG_LEVEL).toBeUndefined();
    expect(subprocessEnv.ANTHROPIC_API_KEY).toBeUndefined();
    // HarneesLab key present
    expect(subprocessEnv.HARNEESLAB_ONLY_KEY).toBe('trusted');
  ...
  it('scenario 4: same key in both CWD and harneeslab env — harneeslab value wins', () => {
    // User has ANTHROPIC_API_KEY in both places. CWD one is the target repo's,
    // harneeslab one is the user's intentional config. HarneesLab must win.
    const subprocessEnv = simulateEntryPointFlow(
      'ANTHROPIC_API_KEY=sk-target-repo-WRONG\nSHARED_KEY=cwd-value\n',
      'ANTHROPIC_API_KEY=sk-my-real-key\nSHARED_KEY=harneeslab-value\n'
    );

    // HarneesLab value wins (CWD was stripped, then harneeslab loaded)
    expect(subprocessEnv.ANTHROPIC_API_KEY).toBe('sk-my-real-key');
    expect(subprocessEnv.SHARED_KEY).toBe('harneeslab-value');
  });
  ...
  it('scenario 5: DATABASE_URL in CWD .env does not reach HarneesLab — harneeslab uses its own DB', () => {
    // Target repo has DATABASE_URL for its own PostgreSQL. HarneesLab must NOT
    // connect to the target app's database — it should use its own DB
    // (from ~/.harneeslab/.env or default SQLite).
    const subprocessEnv = simulateEntryPointFlow(
      'DATABASE_URL=postgresql://target-app:5432/wrong_db\nREDIS_URL=redis://target:6379\n',
      'DATABASE_URL=sqlite:///Users/me/.harneeslab/harneeslab.db\n'
    );

    // CWD DATABASE_URL is stripped, harneeslab's wins
    expect(subprocessEnv.DATABASE_URL).toBe('sqlite:///Users/me/.harneeslab/harneeslab.db');
    // Other CWD keys also stripped
    expect(subprocessEnv.REDIS_URL).toBeUndefined();
  });

  it('scenario 6: DATABASE_URL in CWD .env only (no harneeslab env) — stripped entirely', () => {
    // User relies on default SQLite (no DATABASE_URL in ~/.harneeslab/.env).
  ...
  it('CLAUDE_CODE_OAUTH_TOKEN from harneeslab env survives marker strip', () => {
