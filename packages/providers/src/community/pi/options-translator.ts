 ...
 * HarneesLab's common surface includes 'off' (from Codex's modelReasoningEffort)
 * and 'max' (from Claude's EffortLevel enum). Map into Pi's vocabulary:
 *  - 'off'    → undefined (no explicit thinkingLevel; Pi's implicit off)
 *  - 'max'    → 'xhigh'  (HarneesLab's EffortLevel doesn't have xhigh)
 *  - others pass through if they're already Pi-native
 *
 * See packages/workflows/src/schemas/dag-node.ts#effortLevelSchema for
 * the HarneesLab schema enum (`low | medium | high | max`). Workflow YAML can
 * only carry HarneesLab-enum values; Pi-native `minimal` / `xhigh` are accepted
...
/**
 * Resolve HarneesLab's `effort` / `thinking` node fields to Pi's `ThinkingLevel`.
 *
...
/**
 * Filter Pi's built-in tool set against HarneesLab's `allowed_tools` /
 ...
 *   - allowed_tools: [] → return [] (explicit no-tools; valid HarneesLab idiom)
 ...
 * Claude's SDK and Pi's default resource loader both respect, so HarneesLab
 ...
 * skills win when HarneesLab runs out of a subdirectory.
 */
...
/**
 * Resolve HarneesLab's name-based `skills:` nodeConfig references to absolute
