 ...
 * Callers can set worktree.baseBranch in .harneeslab/config.yaml as a manual override.
 *
    ...
    if (
      ...
      throw new Error(
        `Cannot detect default branch for ${repoPath}: neither origin/HEAD nor origin/main exist. ` +
          'Set worktree.baseBranch in .harneeslab/config.yaml to specify the branch explicitly.'
      );
    }
