...
const ERROR_PATTERNS: { pattern: string; message: string; known: boolean }[] = [
  ...
  },
  {
    ...
    message:
      '**Error:** No base branch configured. Set `worktree.baseBranch` in `.harneeslab/config.yaml` ' +
  ...
  },
  {
    ...
    message:
      ...
      'submodule remotes, or set `worktree.initSubmodules: false` in `.harneeslab/config.yaml` ' +
