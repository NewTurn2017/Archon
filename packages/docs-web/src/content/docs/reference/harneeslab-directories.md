---
title: HarneesLab 디렉터리
description: HarneesLab의 디렉터리 구조, 경로 해석, 설정 시스템을 설명합니다.
category: reference
area: config
audience: [developer]
status: current
...
이 문서는 HarneesLab에 기여하거나 HarneesLab을 확장하는 개발자를 위해 HarneesLab의 디렉터리 구조와 설정 시스템을 설명합니다. HarneesLab은 HarneesLab fork이므로 repo-local `.harneeslab/` workflow convention은 유지하되, user-level runtime env는 `HARNEESLAB_HOME`을 우선 지원합니다.

## 개요

HarneesLab은 다음을 갖춘 통합 디렉터리 및 설정 시스템을 제공합니다.

1. 모든 플랫폼(Mac, Linux, Windows, Docker)에서 **일관된 경로**
2. **설정 우선순위** 체인(env > global > repo > defaults)
3. `.harneeslab/workflows/`의 YAML definition과 연동되는 **workflow engine integration**

## 디렉터리 구조

### 사용자 레벨: `~/.harneeslab/` 또는 `HARNEESLAB_HOME`

기본값은 `~/.harneeslab/`이며, `HARNEESLAB_HOME`이 설정된 경우 그 값이 root가 됩니다.

```
~/.harneeslab/                    # or $HARNEESLAB_HOME
...
- `worktrees/` - `workspaces/` 아래에 등록되지 않은 repo를 위한 fallback
...
### Docker: `/.harneeslab/` 또는 `/.harneeslab/`

새 Docker compose 구성에서는 HarneesLab home이 root level의 `/.harneeslab/`입니다. 이 경로는 다음 특성을 가집니다.
- 지속성을 위해 named volume으로 mount
- `HARNEESLAB_HOME`으로 container 내부 home을 명시적으로 override 가능

## 경로 해석

모든 path resolution은 `packages/paths/src/harneeslab-paths.ts`(`@harneeslab/paths`)에 중앙화되어 있습니다.

### 핵심 함수

```typescript
// Get the HarneesLab home directory
getHarneesLabHome(): string
// Returns: HARNEESLAB_HOME, ~/.harneeslab (local), or Docker home

// Get workspaces directory
getHarneesLabWorkspacesPath(): string
// Returns: ${HarneesLab home}/workspaces

// Get global worktrees directory (fallback)
getHarneesLabWorktreesPath(): string
// Returns: ${HarneesLab home}/worktrees

// Get global config path
getHarneesLabConfigPath(): string
...
| Platform | `getHarneesLabHome()` |
|----------|-------------------|
| macOS | `/Users/<username>/.harneeslab` |
| Linux | `/home/<username>/.harneeslab` |
| Windows | `C:\Users\<username>\.harneeslab` |
...
2. **Global Config** (`~/.harneeslab/config.yaml`) - User preference
3. **Repo Config** (`.harneeslab/config.yaml`) - Project-specific 설정
...
|--------|--------------|---------|
| HarneesLab home | `HARNEESLAB_HOME` | `~/.harneeslab` |
...
commands:
  folder: .claude/commands/harneeslab  # Additional folder to search
```
...
1. `packages/paths/src/harneeslab-paths.ts`에 function 추가:
```typescript
export function getHarneesLabNewPath(): string {
  return join(getHarneesLabHome(), 'new-directory');
}
```

2. `Dockerfile`의 Docker setup 업데이트
3. `docker-compose.yml`의 volume mount 업데이트
4. `packages/paths/src/harneeslab-paths.test.ts`에 test 추가
...
### 왜 `~/.config/harneeslab/` 대신 `~/.harneeslab/`인가?
