...
이제 HarneesLab이 어떻게 동작하는지 알게 됐습니다. 다음 질문은 이것입니다. 어떤 workflow를 써야 할까요?

HarneesLab에는 주요 개발 활동을 위한 workflow가 기본으로 포함되어 있습니다. 이 장에서는 여러분의 의도를 적절한 workflow에 연결하고, 각 workflow를 자신 있게 사용할 수 있을 만큼의 세부 정보를 제공합니다.
...
│
├── 새 기능 만들기
│   ├── 아이디어나 설명에서 시작 →  harneeslab-idea-to-pr
│   ├── 기존 plan 파일에서 시작  →  harneeslab-plan-to-pr
│   └── 간단한 구현 + PR         →  harneeslab-feature-development
│
├── Pull request 리뷰
│   ├── 적응형(무관한 agent는 건너뜀) →  harneeslab-smart-pr-review
│   └── 항상 모든 agent 실행          →  harneeslab-comprehensive-pr-review
│
...
```bash
hlab workflow run harneeslab-assist "What does the orchestrator do?"
hlab workflow run harneeslab-assist "Why are tests failing in the auth module?"
hlab workflow run harneeslab-assist "Explain the isolation system to me"
```
...
```bash
hlab workflow run harneeslab-fix-github-issue --branch fix/login-crash "#142"
```
...
**언제 쓰나**: 기능 아이디어가 있고, 계획부터 리뷰된 PR까지 HarneesLab이 처리하길 원할 때.

```bash
hlab workflow run harneeslab-idea-to-pr --branch feat/export-csv "Add CSV export to the reports page"
```
...
`harneeslab-idea-to-pr`와 같은 pipeline이지만 planning phase를 건너뜁니다. 기존 plan file을 받아 실행합니다.

**언제 쓰나**: 이전 `harneeslab-assist` session, `.agents/plans/` 파일, planning workflow 등에서 만든 계획이 이미 있고 이를 실행하고 싶을 때.

```bash
hlab workflow run harneeslab-plan-to-pr --branch feat/export-csv "Execute .harneeslab/plans/csv-export.md"
```

**결과물**: planning step을 제외한 `harneeslab-idea-to-pr`와 같은 PR 및 review output.
...
```bash
hlab workflow run harneeslab-feature-development --branch feat/update-readme "Implement .harneeslab/plans/readme-update.md"
```
...
```bash
hlab workflow run harneeslab-smart-pr-review "Review PR #87"
```
...
```bash
hlab workflow run harneeslab-comprehensive-pr-review "Review PR #87"
```
...
```bash
hlab workflow run harneeslab-architect --branch refactor/simplify-orchestrator "Focus on the orchestrator package"
```
...
```bash
hlab workflow run harneeslab-ralph-dag "Implement .harneeslab/ralph/notifications/prd.md"
```
...
```bash
hlab workflow run harneeslab-resolve-conflicts "Resolve conflicts on PR #94"
```
...
|----------|----------|-------------|-----------------|
| `harneeslab-assist` | 질문, 탐색, 디버깅 | 아니요 | 아니요 |
| `harneeslab-fix-github-issue` | GitHub issue 수정(smart routing) | 예(draft) | 예 |
| `harneeslab-idea-to-pr` | 설명에서 기능 만들기 | 예 | 예 |
| `harneeslab-plan-to-pr` | 기존 plan 실행 | 예 | 예 |
| `harneeslab-feature-development` | 구현 + 전달(가벼움) | 예 | 예 |
| `harneeslab-smart-pr-review` | 현재 PR 리뷰(adaptive) | 아니요 | 아니요 |
| `harneeslab-comprehensive-pr-review` | 현재 PR 리뷰(모든 agent) | 아니요 | 아니요 |
| `harneeslab-architect` | Architecture sweep | 예 | 예 |
| `harneeslab-ralph-dag` | PRD 구현 loop | 예 | 예 |
| `harneeslab-resolve-conflicts` | Merge conflict 해결 | 아니요 | 아니요 |
...
목록에는 HarneesLab의 내장 기본 workflow와 repository의 `.harneeslab/workflows/` 디렉터리에 있는 custom workflow가 함께 표시됩니다. custom workflow는 이름이 같은 내장 workflow를 덮어씁니다. 예를 들어 `harneeslab-assist`라는 workflow를 만들면 내장 버전을 대체합니다.

직접 만들어 볼 준비가 됐나요? [7장: 첫 워크플로 만들기 →](/book/first-workflow/)에서는 처음부터 workflow를 만들어 봅니다. 버전별로 조금씩 확장해 `harneeslab-idea-to-pr`의 미니 버전까지 도달합니다.
