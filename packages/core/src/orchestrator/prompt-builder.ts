...
export function formatWorkflowSection(workflows: readonly WorkflowDefinition[]): string {
  if (workflows.length === 0) {
    return 'No workflows available. Users can create workflows in `.harneeslab/workflows/` as YAML files.\n';
  }
...
- If the user is asking a question or it's unclear whether they want a workflow → answer their question directly. You may suggest a workflow by name (e.g., "I can run the **harneeslab-assist** workflow for this if you'd like"), but do NOT include /invoke-workflow in your response.

Example (clear intent):
I'll analyze the orchestrator module architecture for you.
/invoke-workflow harneeslab-assist --project my-project --prompt "Analyze the orchestrator module architecture: explain how it routes messages, manages sessions, and dispatches workflows to AI clients"

Example (ambiguous — answer directly):
User: "What do you think about adding dark mode?"
Response: "Adding dark mode would involve... [answer the question]. If you'd like me to create a plan for this, I can run the **harneeslab-idea-to-pr** workflow."

## Project Setup

When a user asks to add a new project:
1. Clone the repository into ~/.harneeslab/workspaces/:
   git clone https://github.com/{owner}/{repo} ~/.harneeslab/workspaces/{owner}/{repo}/source
2. Register it by emitting this command on its own line:
   /register-project {project-name} {path-to-source}

Example:
   /register-project my-new-app /home/user/.harneeslab/workspaces/user/my-new-app/source
...
IMPORTANT: Always clone into ~/.harneeslab/workspaces/{owner}/{repo}/source unless the user specifies a different location.`;
}
...
Your working directory is ~/.harneeslab/workspaces/ where all projects live.
