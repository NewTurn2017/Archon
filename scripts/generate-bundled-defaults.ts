 ...
 * the on-disk defaults in .harneeslab/commands/defaults/ and .harneeslab/workflows/defaults/.
 *
  ...
  try {
    ...
    throw new Error(
      `${label} directory not found: ${dir}\n` +
        ...
        'or verify the .harneeslab/ tree exists.'
    );
  }
}
...
function renderFile(commands: BundledFile[], workflows: BundledFile[]): string {
  const header = [
    ...
    ' *',
    ' * Source of truth:',
    ' *   .harneeslab/commands/defaults/*.md',
    ' *   .harneeslab/workflows/defaults/*.{yaml,yml}',
    ' *',
