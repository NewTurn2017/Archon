...
> **Purpose**: Complete reference for understanding and building platform adapters in HarneesLab.
...
4. Self-loop prevention: ignores comments containing `<!-- harneeslab-bot-response -->`
...
**Sending:** `octokit.rest.issues.createComment` with retry logic (3 attempts, exponential backoff). Appends `<!-- harneeslab-bot-response -->` marker to every comment.
