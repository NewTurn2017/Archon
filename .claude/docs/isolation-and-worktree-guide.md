...
> **Purpose**: Complete reference for how HarneesLab creates, manages, and cleans up git worktrees for parallel development.
...
**2. Sync workspace**
- Calls `syncWorkspace(repoPath, baseBranch)` from `@harneeslab/git/repo.ts`
...
- Default: always copies `.harneeslab/` directory
- User config: additional paths from `.harneeslab/config.yaml` `worktree.copyFiles`
...
- **Project-scoped** (repo under `~/.harneeslab/workspaces/owner/repo/`):
  - Base: `~/.harneeslab/workspaces/owner/repo/worktrees/`
  - Path: `<base>/<branchName>`

- **Legacy global** (repo elsewhere):
  - Base: `~/.harneeslab/worktrees/`
