...
> **Purpose**: End-to-end flow traces across the entire HarneesLab system with file:line references.
    ...
    → lockManager.acquireLock(conversationId, handler) (conversation-lock.ts:59)
      → handleMessage(platform, conversationId, text) (orchestrator-agent.ts:383)
        ...
        ALL OTHER MESSAGES (including unknown slash commands):
          ...
          → cwd = getHarneesLabWorkspacesPath() (orchestrator-agent.ts:458)
...
## 2. Workflow Execution: `/workflow run harneeslab-fix-github-issue #42`
...
```
  ...
  → getWorktreePath() — ~/.harneeslab/workspaces/{owner}/{repo}/worktrees/{branch}
  ...
  → copyConfiguredFiles() — .harneeslab/ + config.worktree.copyFiles
```
...
import * as sessionDb from '@harneeslab/core/db/sessions';
...
Layer 1: Code defaults (config-loader.ts:165)
  → botName: 'HarneesLab', assistant: 'claude', concurrency.maxConversations: 10

Layer 2: Global config (~/.harneeslab/config.yaml)
  → loadGlobalConfig() — cached after first load
  → Overrides: botName, defaultAssistant, assistants.*, streaming modes

Layer 3: Repo config ({repoPath}/.harneeslab/config.yaml)
