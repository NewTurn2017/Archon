---
description: Run HarneesLab's full validation suite with per-level reporting
---

# Validate: Comprehensive HarneesLab Validation

## Objective

Run all four validation levels for the HarneesLab monorepo and report pass/fail with actionable
...
- Package boundary violations (e.g., `@harneeslab/workflows` importing from `@harneeslab/core`)
...
- `@harneeslab/core` — 7 separate `bun test` batches (mock.module pollution prevention)
- `@harneeslab/workflows` — 5 batches
- `@harneeslab/adapters` — 3 batches
- `@harneeslab/isolation` — 3 batches
...
```bash
bun --filter @harneeslab/workflows test
```
