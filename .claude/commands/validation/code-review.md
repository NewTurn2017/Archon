...
Perform a thorough technical code review on recently changed files, checking for bugs, security issues, and adherence to HarneesLab's documented conventions.
...
**HarneesLab-Specific Conventions**
...
- `@harneeslab/git` and `@harneeslab/paths` must not import from `@harneeslab/core`
- `@harneeslab/workflows` injects deps via narrow interfaces, not direct core imports
