...
**URLs**:
- https://github.com/coleam00/HarneesLab/issues/1001
- https://github.com/coleam00/HarneesLab/issues/1002
- https://github.com/coleam00/HarneesLab/issues/1003
**Type**: BUG (all three)
...
The `harneeslab-interactive-prd` workflow has three bugs that together make it completely non-functional:
...
Evidence: `.harneeslab/workflows/defaults/harneeslab-interactive-prd.yaml:52-55, 106-109, 172-175`
...
Evidence: `.harneeslab/workflows/defaults/harneeslab-interactive-prd.yaml:191-196`
```yaml
...
`$ARTIFACTS_DIR` is pre-created by the executor at `packages/workflows/src/executor.ts:468` and is always writable (resolves to `~/.harneeslab/workspaces/{owner}/{repo}/artifacts/runs/{runId}/`).
...
|------|-------|--------|-------------|
| `.harneeslab/workflows/defaults/harneeslab-interactive-prd.yaml` | 52-55, 106-109, 172-175, 191-196, 246-249 | UPDATE | Add `capture_response: true` to gates; change `.claude/PRPs/prds/` to `$ARTIFACTS_DIR/prds/` |
...
**File**: `.harneeslab/workflows/defaults/harneeslab-interactive-prd.yaml`
**Action**: UPDATE
...
1. **YAML validation**: Run `bun run cli validate workflows harneeslab-interactive-prd` to confirm the YAML is valid after changes
...
bun run test
bun run lint
bun run cli validate workflows harneeslab-interactive-prd
```

### Manual Verification

1. Run `bun run cli workflow run harneeslab-interactive-prd "Build a todo app"` — verify it pauses at foundation-gate
...
**IN SCOPE:**
- Adding `capture_response: true` to three approval gates in `harneeslab-interactive-prd.yaml`
