...
description: |
  Replicate and validate a GitHub issue by spinning up HarneesLab, analyzing the issue,
  ...
  Capability: Checks out main, pulls latest, starts HarneesLab, reads the GitHub issue,
  then uses agent-browser to systematically test every symptom and produce a findings report.
  NOT for: Fixing issues (use /harneeslab or /exp-piv-loop:fix-issue), general UI testing (use /validate-ui).
...
Systematically reproduce and validate a GitHub issue against the live HarneesLab application.
...
### 0.2 Kill Existing HarneesLab Processes

Free up ports 3090 (backend) and 5173 (frontend) so HarneesLab starts cleanly.
...
# Trigger a workflow (harneeslab-assist is a good general-purpose one)
curl -s -X POST http://localhost:3090/api/workflows/harneeslab-assist/run \
...
# Stop HarneesLab (optional — leave running if user wants to continue testing)
