...
| Rule | Violation |
|------|-----------|
| No generic `import *` from `@harneeslab/core` | `import * as core from '@harneeslab/core'` |
| Use `import type` for type-only | `import { MyType } from '...'` when MyType is only used as a type |
| Namespace imports for submodules only | `import * as conversationDb from '@harneeslab/core/db/conversations'` is fine |
| Specific named imports for values | `import { handleMessage, pool } from '@harneeslab/core'` |
...
| Use `@harneeslab/git` functions | Direct git shell calls when an `@harneeslab/git` function exists |
...
- **Generic `import *`** — from `@harneeslab/core` instead of specific named imports
