...
| `.harneeslab/ralph/{slug}/prd.md` exists | Existing PRD in ralph dir | Generate prd.json alongside it |
...
```bash
mkdir -p .harneeslab/ralph/{slug}
```

### 5.3 prd.md 작성

**Output path**: `.harneeslab/ralph/{slug}/prd.md`
...
**Output path**: `.harneeslab/ralph/{slug}/prd.json`
...
```bash
git add .harneeslab/ralph/{slug}/
git commit -m "docs: add Ralph PRD for {feature name}"
```

**PHASE_5_CHECKPOINT:**
- [ ] `.harneeslab/ralph/{slug}/prd.md` written with all sections
- [ ] `.harneeslab/ralph/{slug}/prd.json` written with properly sized stories
...
**Feature**: {name}
**Directory**: `.harneeslab/ralph/{slug}/`
...
- **FILES_WRITTEN**: Both prd.md and prd.json exist in `.harneeslab/ralph/{slug}/`
