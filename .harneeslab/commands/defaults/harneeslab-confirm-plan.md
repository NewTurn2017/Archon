...
Run harneeslab-plan-setup first.
```
...
Continue to `harneeslab-implement-tasks` to execute the plan.

{If STOP:}
Revise the plan to address blockers, then re-run `harneeslab-plan-setup`.
```
...
Proceed to `harneeslab-implement-tasks` to execute the plan.
```
...
3. **Re-run planning** with `/harneeslab-plan` to generate a fresh plan
